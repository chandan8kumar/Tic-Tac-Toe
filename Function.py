...
def defend(letter,a,b,c,inputs,input_list):
    ...
    p=defend_row(a,b,c,letter,inputs,input_list)
    if p==1:
        return 0
    q=defend_col(a,b,c,letter,inputs,input_list)
    if q==1:
        return 0
    r=defend_diag(a,b,c,letter,inputs,input_list)
    if r==1:
        return 0
    return 1
def defend_col(a,b,c,letter,inputs,input_list):
    '''Checks for the next move in COLUMNS if found then put a opposite sign on next move and return 1 else return 0'''
    for i in range(3):
        if a[i]==b[i]:
            if c[i]!='X' or c[i]!='O':
                temp=inputs.get(c[i])
            ...
            if b[i]!='X' or b[i]!='O':
                ...
                if temp in input_list:
                    ...
                    return 1
    return 0
def defend_row(a,b,c,letter,inputs,input_list):
        ...
        if a[i]==a[i+1]:
            ...
            if c[i+1]!='X' or c[i+1]!='O':
                ...
                if temp in input_list:
                    ...
                    return 1
    return 0
def defend_diag(a,b,c,letter,inputs,input_list):
    '''Checks for the next move in DIAGONALS if found then put a opposite sign on next move and return 1 else return 0'''
    for i in range(1):
        if a[i]==b[i+1]:
            if c[i+2]!='X' or c[i+2]!='O':
                temp=inputs.get(c[i+2])
